...
if __name__ == "__main__":
    ...
    vacuum_full_path = vacuum_data_path

    smd_data_path = os.path.join(base_path, "smd")
    smd_full_path = smd_data_path
    ...
    for entry in tqdm(
        ...
        # Make sure that we're not somehow missing atoms in charges, forces
        # Will check partial spins later
        if any(
            [
                len(item[x]) != length
                for x in [
    ...
    # Neutral
    vacuum_full_neutral_path = os.path.join(vacuum_full_path, "neutral")
